...
#include "generalised_stokes_with_variable_viscosity.h"
...
GeneralisedStokesWithVariableViscosity<TDim>::GeneralisedStokesWithVariableViscosity(IndexType NewId)
    : Element(NewId)
{}

template< unsigned int TDim >
GeneralisedStokesWithVariableViscosity<TDim>::GeneralisedStokesWithVariableViscosity(
    IndexType NewId,
...
template< unsigned int TDim >
Element::Pointer GeneralisedStokesWithVariableViscosity<TDim>::Create(
    IndexType NewId,
    NodesArrayType const& ThisNodes,
    Properties::Pointer pProperties) const
{
    return Kratos::make_intrusive<GeneralisedStokesWithVariableViscosity<TDim>>(NewId, this->GetGeometry().Create(ThisNodes), pProperties);
}
...
Element::Pointer GeneralisedStokesWithVariableViscosity<TDim>::Create(
    ...
    Properties::Pointer pProperties) const
{
    return Kratos::make_intrusive<GeneralisedStokesWithVariableViscosity<TDim>>(NewId, pGeom, pProperties);
}

template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::EquationIdVector(
...
template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::GetDofList(
...
template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::CalculateLocalSystem(
...
template< unsigned int TDim >
int GeneralisedStokesWithVariableViscosity<TDim>::Check(const ProcessInfo &rCurrentProcessInfo) const
{
    KRATOS_TRY;
...
template< unsigned int TDim >
const Parameters GeneralisedStokesWithVariableViscosity<TDim>::GetSpecifications() const
{
...
template< unsigned int TDim >
std::string GeneralisedStokesWithVariableViscosity<TDim>::Info() const
{
    std::stringstream buffer;
    buffer << "GeneralisedStokesWithVariableViscosity" << TDim << "D" << NumNodes << "N #" << this->Id();
    return buffer.str();
}

template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::PrintInfo(std::ostream& rOStream) const
{
...
template <unsigned int TDim>
void GeneralisedStokesWithVariableViscosity<TDim>::SetElementData(
    ...
    ElementDataContainer &rData)
{
    ...
    rData.Delta = rProcessInfo.GetValue(TAUONE); // It would be better to define a unique new variaqble for this parameter
...
template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::CalculateKinematics(
...
template <>
void GeneralisedStokesWithVariableViscosity<2>::AddGaussPointLeftHandSideContribution(
...
template <>
void GeneralisedStokesWithVariableViscosity<3>::AddGaussPointLeftHandSideContribution(
    const ElementDataContainer& rData,
    MatrixType& rLeftHandSideMatrix)
{
    //Get nodal data
...
template <>
void GeneralisedStokesWithVariableViscosity<2>::AddGaussPointRightHandSideContribution(
    const ElementDataContainer& rData,
    VectorType& rRightHandSideVector)
{
    //Get nodal data
...
template <>
void GeneralisedStokesWithVariableViscosity<3>::AddGaussPointRightHandSideContribution(
    const ElementDataContainer& rData,
    VectorType& rRightHandSideVector)
{
    //Get nodal data
...
template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::save(Serializer& rSerializer) const
{
...
template< unsigned int TDim >
void GeneralisedStokesWithVariableViscosity<TDim>::load(Serializer& rSerializer)
{
...
template class GeneralisedStokesWithVariableViscosity<2>;
template class GeneralisedStokesWithVariableViscosity<3>;
