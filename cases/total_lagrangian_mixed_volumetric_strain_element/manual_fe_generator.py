...
def main(options_dict):
    ...
    # Define shape functions and derivatives (method in Kratos utils)
    N,DN = ksu.DefineShapeFunctions(n_nodes, dim, impose_partion_of_unity = False)

    # Symbols definition
    u = ksu.DefineMatrix('u_nodes',n_nodes,dim) # Displacement (u(i,k) refers to the displacement of node i component k)
    ## CHANGE DEFINITION OF "th"
    # th = ksu.DefineVector('theta_nodes',n_nodes) # Variable representing the nodal det(J) - 1 (volumetric deformation increment)
    # So, originally represents volumetric strain (e_v). We know that e_v = J - 1
    th = ksu.DefineVector('theta_nodes',n_nodes) # Variable representing an alternative version of J (detF) used as an unknown
    w = ksu.DefineMatrix('w_nodes',n_nodes,dim) # Displacement test function
    q = ksu.DefineVector('q_nodes',n_nodes) # Deformation Jacobian test function
    w_g = sympy.Symbol("w_g", positive=True) # Gauss point integration weight
    tau_u = sympy.Symbol("tau_u_gauss",positive=True) # Displacement stabilization constant
    tau_th = sympy.Symbol("tau_theta_gauss",positive=True) # Deformation Jacobian stabilization constant

    S = ksu.DefineVector('S_gauss',strain_size) # Stress in Voigt notation (this will be returned by the constitutive law)
    if dim == 2:
        S = sympy.Matrix([
            [S[0],S[2]],
    ...
    C = ksu.DefineSymmetricMatrix("C_gauss",strain_size,strain_size) # Constitutive matrix in Voigt notation (this will be returned by the constitutive law)
    C = ksu.ConvertVoigtMatrixToTensor(C) # Definition of the 4th order constitutive tensor from the previous definition symbols
    ...
    # Define the body force and density interpolation at the Gauss point
    b_gauss = ksu.DefineVector('b_o_gauss', dim)
    rho_gauss = sympy.Symbol('rho_o_gauss', positive=True)
    ...
    Fmod_gauss = (th_gauss**sympy.Rational(1,dim))*Fbar_gauss # Equivalent (enriched) deformation gradient tensor
    ...
    # Variational form
    tmp = (ksu.DoubleContraction(C, F_gauss.transpose()*F_gauss)).tomatrix()

    mom_first = ksu.DoubleContraction(grad_w_gauss, F_gauss* S)
    mom_second = (w_gauss.transpose() * rho_gauss * b_gauss)[0]
    mom_aux_scalar = (tau_th / dim) * (th_gauss**sympy.Rational(2-dim,dim)) * (1.0/j_gauss**sympy.Rational(2,dim))
    mom_stab = ksu.DoubleContraction(grad_w_gauss, mom_aux_scalar * (th_gauss - j_gauss) * tmp)

    mass_first = (1.0 - tau_th) * q_gauss[0] * (th_gauss - j_gauss)
    mass_aux_scalar = (tau_u / dim) * ((j_gauss / th_gauss)**sympy.Rational(dim-2,dim))
    mass_stab_1 = (mass_aux_scalar * grad_q_gauss * tmp * grad_th_gauss)[0]
    mass_stab_2 = (tau_u * grad_q_gauss * cofF_gauss.transpose() * rho_gauss *b_gauss)[0]
    ...
    rhs = ksu.Compute_RHS(functional_array.copy(), testfunc, do_simplifications = False)
    rhs_out = ksu.OutputVector_CollectingFactors(w_g*rhs, "rRightHandSideVector", "c", indentation_level=2, assignment_op="+=")
    ...
    for i in range(n_dofs):
        for j in range(n_dofs):
                lhs[i,j] -= sympy.diff(lhs_rhs[i], dofs[j])
                lhs[i,j] = lhs[i,j].subs(substitution_list)
    lhs_out = ksu.OutputMatrix_CollectingFactors(w_g*lhs, "rLeftHandSideMatrix", "c", indentation_level=2, assignment_op="+=")
